...
class PopulateJsonFireStore {
  ...
  async populate() {
    ...
    for (var item of data) {
      ...
      if (data.length - 1 === i) {
        console.log(`\nUpload is Successed!`);
        console.timeEnd("The passing time");
        this.exit(0);
      }
      i++;
    }
  ...
  async add(item) {
    ...
    try {
      await this.db.collection(this.collectionname).add(Object.assign({}, item));
      return true;
    } catch (e) {
  ...
  async set(item) {
    ...
    try {
      await this.db.doc(`${this.collectionname}/${item.id}`).set(Object.assign({}, item));
      return true;
    } catch (e) {
      return console.error(e.message);
    }
...
// command to run
//node bulk-json-to-firebase.js demo-data.json add demo-name